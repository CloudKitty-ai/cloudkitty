...
|------|--------|------------|---------------|
| `fog-gen1-miso-cand-s2.ckpolicy` | `75abb7d948410e5634d0a09cb536ce42573d250a3543fdc66b1fd004aa0d83a8` | **Fog Gen 1, Miso's seat** (trained as `cand-s2`) — step-7 BC+PPO: lesson clone of the Biscuit 3.0 corpus, leash β 0.04, run indices 21–40, native schema 5 (obs 408). `export_v5.py` dump of the battery-measured torch actor; parity **2.4e-05 max logit delta** over 2,000 real probe rows, exact argmax agreement on both heads under the mask (flipped-byte negative control 3.6e-02); `.parity.json` beside the handoff copy. **Seated at the 0.3.0 cutover** on the owner's word (roster gen1-A, 2026-09-15). Greedy selection. | `experiments/fog-gen1-cert/RESULTS.md` — battery all gates on four bands under the trained clock (0/120 runs ≥ distress age 150); handoff `seating-handoff-2026-09-16.md`; prereg `PREREG.md` |
| `fog-gen1-biscuit-cand-s1.ckpolicy` | `cecd89c8c53dd2ed28e81a778cc9e3a685cf18e21236fcca100f63a5708c2ca8` | **Fog Gen 1, Biscuit's seat** (trained as `cand-s1`) — same recipe and export path as `fog-gen1-miso-cand-s2` (leash β 0.04); parity **2.2e-05** (control 1.8e-01). **Seated at the 0.3.0 cutover** (roster gen1-A). Greedy selection. | Same records: `experiments/fog-gen1-cert/RESULTS.md`, `seating-handoff-2026-09-16.md` |
| `fog-gen1-pumpkin-dose-lo-s1.ckpolicy` | `02a36461c34c15354f069aa6c2a915af9f13a1d8698f51cf31380c7e6cc19c97` | **Fog Gen 1, Pumpkin's seat** (trained as `dose-lo-s1`, the low-dose arm: leash **β 0.02**) — otherwise the same recipe and export path; parity **3.6e-05** (control 4.8e-01). **Seated at the 0.3.0 cutover** (roster gen1-A). Greedy selection. | Same records: `experiments/fog-gen1-cert/RESULTS.md`, `seating-handoff-2026-09-16.md` |
| `fog-gen1-kittybear-cand-s4.ckpolicy` | `3aa42dd4cb13b2e5fcd18f35da06345b22e636a12939d5b639f061276b792ee0` | **Fog Gen 1, Kittybear's seat** (trained as `cand-s4`) — same recipe and export path (leash β 0.04); parity **1.9e-05** (control 1.3e-01). **Seated at the 0.3.0 cutover** (roster gen1-A). Greedy selection. | Same records: `experiments/fog-gen1-cert/RESULTS.md`, `seating-handoff-2026-09-16.md` |
| `fog-gen1-clementine-cand-s7.ckpolicy` | `507525314b35587d758e05736c671d60d74c8b4dd3896ffba62a92a3c7c4f1ae` | **Fog Gen 1, Clementine's seat** (trained as `cand-s7`) — same recipe and export path (leash β 0.04); parity **1.6e-05** (control 1.0e-01). **Seated at the 0.3.0 cutover** (roster gen1-A). Greedy selection. | Same records: `experiments/fog-gen1-cert/RESULTS.md`, `seating-handoff-2026-09-16.md` |
...
|------|--------|---------|---------------|
| `retired/attn-a1-s1-o4.ckpolicy` | `61d6d7cc699f1de303b4fb661a77380bf56b5d69e76db3eac5bd316b38ed604a` | "The cuddler" (`attn-a1-s1` below) carried onto the schema-4 surface by `ckpolicy-expand` v1 (spec 035; mapped 77083, zeroed 448, floored 903 — structural attestation + bit-exact old-dim parity, `expansion-acceptance-2026-08-17.md`). Drove Miso — the seat this mind held since candidate B — from the 2026-08-21 phase-1 cutover until the 3.0 wall (spec 049, #344, 2026-09-04): observation schema 5 refuses schema 4, and every seat served scripted through the wall's interregnum. Certified in `deploy-battery-2026-08-21.md`, all gates. **Retired on the owner's word 2026-09-16** ("Retire 2.x artifacts") after the 0.3.0 cutover. | `fog-gen1-miso-cand-s2.ckpolicy` (#377, the 0.3.0 seating) |
| `retired/attn-a1-s3-o4.ckpolicy` | `d6f60818ad0516445367a3cdbca2a7df24a36886ed457e3ee1c8fe06004569ad` | "The doter" (`attn-a1-s3` below) on the schema-4 surface (same expansion tooling and records). Drove Pumpkin, **seated ONCE** — the twin-seating pattern retired with F-027 (the r5 co-sleep deadlock was a twin artifact) — from 2026-08-21 until the 3.0 wall. Certified in `deploy-battery-2026-08-21.md`, all gates. Retired 2026-09-16. | `fog-gen1-pumpkin-dose-lo-s1.ckpolicy` (#377) |
| `retired/e004-a1-s2-o4.ckpolicy` | `b6293849a63bd2f8b915080e74a20a5dd5f539eb48911bece3d4e23876588b09` | The exp-004 MLP incumbent (`e004-a1-s2` below) on the schema-4 surface (mapped 127531, zeroed 7168, floored 1799; structurally deaf to the new kinds — its 28 new input columns provably zero). Drove Biscuit from 2026-08-21, then **Clementine from 2026-08-22** — the fifth cat's first mind, ending her scripted phase (battery 94.81 vs scripted bar 90.10, `battery-2026-08-22.md`) — until the 3.0 wall. Retired 2026-09-16. | `fog-gen1-clementine-cand-s7.ckpolicy` (#377) |
| `retired/e006-E1-s1.ckpolicy` | `a7ea333824b2a2742f4ea71f15f301775ef6ca167e291c72a22d0bb6cf7b8e3a` | exp-006 E-arm PPO product (Biscuit-clone init, actor-only export; forward parity 7.8e-05) — the first natively schema-4 mind to serve, seated for its dialect (here-meows and a meaningful mew; the swap free at team level, G2c +0.0003). Drove Kittybear from 2026-08-21 until the 3.0 wall. Certified in `deploy-battery-2026-08-21.md`, all gates. Retired 2026-09-16. | `fog-gen1-kittybear-cand-s4.ckpolicy` (#377) |
| `retired/e006a-L-04-s3.ckpolicy` | `a32c457e247e19cc894c05cb876daa79cd6a7f2425bafe74b1fcd5bbe9497558` | **Biscuit 2.0** — exp-006a lineage PPO product (clone-anchor-v6 init/leash β∞ 0.04, dataset v6, trained on family-spread-bugs2; plain-V4 actor export, parity 2.29e-05, `make_oracle_v4`). Drove Biscuit from 2026-08-22 — the seat's own BC lineage grown up (battery 89.95 vs the owner's bar 87.31, G3 fingerprint PASS, `battery-2026-08-22.md`) — until the 3.0 wall; the Biscuit 3.0 playful anchor (spec 049 T091) held the seat through the interregnum, and the Gen 1 mind is a lesson clone of that anchor's corpus. Retired 2026-09-16. | `fog-gen1-biscuit-cand-s1.ckpolicy` (#377) |
...
**All twelve retired artifacts are a generation (or more) behind the
binary and can never run again here.** The five retired at the 0.3.0
cutover (owner's word 2026-09-16) pin observation schema 4 against a
binary that speaks 5; the three retired at the phase-1 cutover pin
3/2/2 against 4/3/3; the four oldest predate the in-water observation
bit (schema 1 → 2). No retired mind serves on through a successor any
more: the fog Gen 1 generation is trained fresh under fog, not
expanded from these. `PolicyArtifact::load` rejects every file in this
table independently of this README. They are kept because they ran the
world, not because they could.
