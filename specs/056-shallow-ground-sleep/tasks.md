...
- [x] T004 Validation: the key joins the `[actions]` finite/≥0 sweep (~line 746) AND one cross-field check `sleep_floor_off_beam < [thresholds] distress` (configured value, never literal 90) beside the existing cross-section checks; unit tests refuse a negative floor and a floor ≥ the configured distress; mutate red per plan ledger item 6 (drop the cross-check → predict the refusal test fails), in crates/cloudkitty-core/src/config/validate.rs
