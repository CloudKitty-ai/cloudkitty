...
- **Reverse race (review finding 2, documented)**: in the rescue-forgone case the old scene is still alive at the cat's apply slot (the friend came back), so if the *substitute* action happens to fail validation (a move into a just-taken tile, a bowl emptied earlier the same tick), the refusal is recorded against the still-live scene as `absorbed=true` — a row class that did not exist pre-048. Bounded by the rescue-forgone volume (6–9 per 20k-tick reference run) times the substitute's validation-failure odds; lands in the `absorbed=true` class the refusal-tax instrument already excludes. Documented for the measurement re-baseline rather than prevented.
