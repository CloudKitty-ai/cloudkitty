...
- [X] T001 Create `policies/registry.toml` with the three ship rows exactly per contracts/registry-and-serving.md §1–2 (sha-keyed tables, header comment block stating the born-in-the-artifact-PR / never-changes rules, plus the FR-002 anticipated forward values as comments — `"Scripted"` is fixed in code, `"Transformer · BC+PPO+leash"` expected for phase-1 lineage seats if the leash doctrine lands — documentation, not rows)

## Phase 2: Foundational (blocking prerequisites)

- [X] T002 [P] Add `behavior_description: Option<String>` to `Kitty` in crates/cloudkitty-core/src/kitty.rs with `#[serde(default, skip_serializing_if = "Option::is_none")]` and a doc comment mirroring `behavior`'s: server-stamped presentation, registry/config authoritative on resume, never read by the engine (research D3)
- [X] T003 [P] Registry loader in crates/cloudkitty-server/src/lib.rs: strict-parse structs for the file shape (deny_unknown_fields; required non-empty `architecture`/`recipe`/`display`), `fn load_registry_beside(artifact: &Path)` resolving `registry.toml` in the artifact's parent dir (research D2), typed errors distinguishing missing-file / parse / missing-row / empty-field; unit tests in-file for each error and the happy path
...
- [X] T011 [P] CHANGELOG.md one-liner under `## Unreleased` (no `[stamp]`/`[obs-schema]` markers — research D7): registry + served behavior descriptions, spec 034
...
- **T007**: the re-stamp lives in `main.rs` as one `stamp_behavior_descriptions`
  call after `load_or_generate_world`, covering fresh, `--fresh`, and resume
  through a single call site — not inside the persist loader (whose signature
  stays untouched). The task's intent holds in full: the stamp overwrites
  unconditionally (including to `None` for plugin seats, tested in
  persist.rs), and the pre-034/stale-snapshot cases are tested exactly as
  written. Smaller footprint than the planned signature change.
- **T004/T009 fixture side-effect**: FR-007 made four pre-existing policy
  tests need registry rows beside their fixture artifacts — added via a new
  shared `test_support::registry_row_beside` helper with per-test fixture
  dirs; no pre-existing assertion changed.
