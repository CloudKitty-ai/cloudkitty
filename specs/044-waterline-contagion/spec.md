...
- Q: When only one side of a paired scene names the partner in its own activity — a groomer grooming an idle cat — should the referenced cat (the idle groomee) also pay the contagion charge when its groomer is standing in water? → A: No — own-activity only (Option A). A cat pays only when *its own* activity names an in-water partner; a merely-referenced cat pays nothing. Social play is reciprocal by construction, so both members name each other; rest, co-sleep, and groom charge the naming side only. *(Corrected 2026-08-31, review finding 8: the original answer said both play members "pay" — naming is mutual, but at most the dry member pays, since a wet member pays occupancy instead and a scene has at most one dry-beside-wet member.)* This also makes stacking impossible (one partner per activity → at most one contagion charge per cat per tick), keeping the per-tick worst case and the validation budget exactly as FR-009 states them.
