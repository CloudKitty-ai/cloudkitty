...
- [X] T008 [US2] Deaf/mute behavioral tests in crates/cloudkitty-rl/tests/expansion.rs per the U1 per-family split: (a) forward-level fuzz — expanded synthetic mind's message-head argmax is never a post-wall kind across seeded-varied observations (SimRng); v2: logits bit-identical when new-kind digest features vary between zero and nonzero (full deafness); v3: logits bit-identical under any RELABELING of new kinds in the digest (kind-identity insensitivity — swap which new kind carries an injected tuple, forward unchanged); (b) world-level — two-kitty world with full vocabulary enabled, expanded v2 mind seated as a behavior, neighbor's new-kind meow injected into world state, tick forward: zero post-wall-kind emissions and decisions unchanged vs the uninjected world (v2's full-deaf form; the v3 residual is Experiments' registered acceptance-QA measurement, not an in-repo assertion)
- [X] T009 [US2] Attestation mutation tests in crates/cloudkitty-rl/tests/expansion.rs: a deliberately corrupted expansion (one new head bias set to 0.0; one new input parameter set nonzero; one source parameter dropped) each produce verdict FAIL naming the violated class — the attestation cannot be satisfied by accident (SC-004)
