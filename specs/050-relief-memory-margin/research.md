...
- **Decision**: three layers.
  ...
  3. **Integration (new `tests/relief_memory_margin.rs`, SC-004)**: served `cloudkitty.toml` all-scripted (the four-file house pattern: read, retarget behaviours to `needs_driven`, validate), radius asserted 5, `World::tick` × 20,000 (the house stream horizon — the served rate is ~1.2 drink calls per 1,000 with the first at tick 1,610, so 1,000 ticks read 0 on the served seed; measured at implement time); count new meows per tick by kind. Assert `want_drink > 0` on the served config verbatim. Second run with `reply_intensity_floor = Some(0.01)` (any value > 0 — clarification 1; 0.01 so the floor's value cannot starve the count and 0.30 is not encoded): assert `here_water` with `reply == true` > 0. Print drink and eat counts as the F-040 reading. The red for this guard is the engine with the key parsed but unread (the field lands before the predicate) → 0 drink calls.
