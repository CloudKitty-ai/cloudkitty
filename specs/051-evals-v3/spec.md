...
**Independent Test**: a fixture artifact at the served observation width (the test-support fixture writer already produces one from the compiled defaults) is run against the landed `evals/v3` through the suite loader and scorer; every exam gets past artifact load and produces a scored outcome. The same test against `evals/v2` fails on the four wide exams — that is the guard seen red first.
...
2. **Given** the same artifact, **When** the suite run is invoked on `evals/v2`, **Then** the four wide exams refuse it at load (the unchanged v2 behaviour, recorded as the reason v3 exists).
...
3. **Given** `scarcity` and `heterogeneity`, **When** compared with v2, **Then** every section below the head comment is byte-identical; only the head comment's file name and suite version moved.
...
- **FR-004**: `scarcity` and `heterogeneity` are carried: byte-identical below the head comment, whose file name and suite version are the only changes. Their hashes are new because the header moved.
...
- **FR-006**: A policy artifact at the served observation width loads on every v3 exam and the suite scores all six. This is asserted by a test that runs a served-width fixture artifact through the suite; the same fixture against v2 is refused on the four wide exams.
...
- **SC-001**: A served-width policy artifact runs through all six `evals/v3` exams to a scored outcome; the same artifact is refused by 4 of 6 `evals/v2` exams. The v3 assertion is seen red first (against v2, or against a v3 draft with a v2 roster) before it is green.
...
- **SC-006**: The two carried exams differ from their v2 originals only in the head comment (asserted by diffing the files below the header).
...
- Carried exams are re-headed rather than byte-copied: a v3 file whose head comment says "eval-suite-v2" would misname itself. The cost is two new hashes; the body is asserted byte-identical.
