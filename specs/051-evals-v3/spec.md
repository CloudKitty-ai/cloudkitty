...
**Independent Test**: the freeze guard, the manifest loaders, the threshold and sign-test derivations and the sweep assertion all read v3; `evals/v2` stays in the shipped-config sweeps while its bytes load (owner ruling 2026-09-05, code-review finding 1: v2's bytes still load on this engine, so it stays in the sweeps; it joins the exclusions only on the day it stops loading, the v1 precedent, with the carried-equality guard deleted in the same change); the usage string and the training doc name v3; no byte of `evals/v2` moves.
...
3. **Given** the shipped-config sweep, **When** it runs, **Then** `evals/v3` is in scope and loads on the current engine, and `evals/v2` still loads in the same sweep — a record whose bytes load stays in scope; it leaves the sweep only when it stops loading, as v1 did (owner ruling 2026-09-05, code-review finding 1: v2's bytes still load on this engine, so it stays in the sweeps; it joins the exclusions only on the day it stops loading, the v1 precedent, with the carried-equality guard deleted in the same change).
...
- **FR-009**: the sweep assertion that the frozen exams are in scope names `evals/v3`. `evals/v2` is NOT added to `config-sweep-exclusions.txt`: its bytes load on this engine, and the exclusions manifest is reserved for records that no longer do (owner ruling 2026-09-05, code-review finding 1: v2's bytes still load on this engine, so it stays in the sweeps; it joins the exclusions only on the day it stops loading, the v1 precedent, with the carried-equality guard deleted in the same change).
...
- **Record suite**: a landed version no longer current (v1 the 2.x record, v2 the pre-ruling 3.0 record): bytes never move, results historical; it leaves the shipped-config sweep only when its bytes stop loading (v1 did; v2 has not).
...
- **SC-005**: The shipped-config sweep loads `evals/v3` and `evals/v2` on the current engine; the sweep's "frozen exams are in scope" assertion names v3 and is seen red pointed at a directory that does not exist (owner ruling 2026-09-05, code-review finding 1: v2's bytes still load on this engine, so it stays in the sweeps; it joins the exclusions only on the day it stops loading, the v1 precedent, with the carried-equality guard deleted in the same change).
...
- **SC-008**: a served-width policy subject against a scratch suite whose exam seats 6 cats is refused before any tick with a message naming the exam, roster 6 and slots 4; a built-in subject on the same scratch suite scores it; the refusal guard is seen red on the unchanged engine (the run scores instead of refusing).
...
- No observation, artifact or wire change: schema 5 stays 408 floats. The suite loader, scorer and verdict code are untouched except for FR-012 (one roster-fit check at the subject seam, ruled in 2026-09-05). If the loader needs any other change to run v3, that is a finding to report, not a change to make.
...
- Test names that still say "v1" (`every_v1_exam_sustains_an_invariant_asserted_run`) were reported, not renamed (CLAUDE.md rule 3); the owner ruled the rename in at converge time (2026-09-05): it is now `every_exam_sustains_an_invariant_asserted_run`, with the 017 quickstart map updated.
