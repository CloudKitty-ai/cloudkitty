...
- [ ] T012 [US1] Add `SuiteRunError::RosterOverflow { exam: String, roster: usize, slots: usize }` in `crates/cloudkitty-rl/src/suite.rs` with a Display line "exam '{exam}': a roster of {roster} kitties needs {roster-1} kitty slots; the policy subject observes {slots} (spec 051 FR-012) — a wider roster would be truncated silently from its view"; in `score_suite`, BEFORE the scoring loop and only when `subject.is_policy`, walk every `LoadedExam::Standard` core and every `LoadedExam::MixedRoster` cell core and return the error on the first `core.kitties.len() > RlConfig::default().observation.kitty_slots + 1` (doc comment: the subject is bound at the compiled default by `kitty-eval`'s `resolve_subject`; if a suite subject ever carries its own `RlConfig`, read that); in `crates/cloudkitty-rl/src/bin/kitty-eval.rs` map the new variant to exit 1 with the message on stderr (find the existing `SuiteRunError` match and add the arm; read how `Determinism` is mapped and keep its code unchanged).
- [ ] T013 [US1] Add test `a_policy_subject_that_cannot_seat_the_roster_is_refused` to `crates/cloudkitty-rl/tests/eval_suite.rs` (SC-008): build a scratch suite from v3 via `build_scratch_suite` but with `scale.toml`'s text given a sixth `[[kitty]]` (id 6, "Mochi", x 24, y 6, `needs_driven`) and `kitty_slots = 5` (rehash that file); bind the T010 fixture artifact as a policy subject (`SuiteSubject { is_policy: true, .. }`); `score_suite(&suite, &subject, false)` → `assert!(matches!(err, SuiteRunError::RosterOverflow { roster: 6, slots: 4, .. }))` and the message names "scale"; then the same scratch suite with `registry_with_candidate("needs_driven")`, `is_policy: false` → `Ok(_)` (a built-in scores it).
- [ ] T014 [US1] Run T013 on the unchanged engine FIRST (stash T012 or write T013 before T012 — write the test first, commit it red): predict RED — `score_suite` returns `Ok` (the run scores the truncated view); record as cycle U2 in `redden-list.md`; land T012; predict GREEN on T013, and `an_artifact_named_candidate_does_not_panic_the_suite` still GREEN (its scratch suite is v3, rosters ≤ 5); run `cargo test -p cloudkitty-rl`; record; commit.
...
- [ ] T027 `scratchpad/cycle.sh final`; predict: baseline count + 3 new tests (T010, T013, T016), 0 failures, 6 ignored; READ the count; close `redden-list.md` with the final table (cycles F0, U1–U7, final) and the R1 record; commit.
...
- FR-012 (T012–T014) is independent of the exam files and may run before T010 if convenient, but T013's scratch suite reads v3 (Phase 2 done).
