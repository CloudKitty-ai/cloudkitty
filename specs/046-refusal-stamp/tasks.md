...
- [ ] T002 Add `RefusalEvent` struct (`kitty_id: KittyId, proposed: Action, tick: u64, absorbed: bool` — `absorbed` always serialized, no skip; derives per data-model.md, `Copy` iff `Action: Copy`) and `pub type RefusalLog = EventLog<RefusalEvent>` in `crates/cloudkitty-core/src/events.rs`
...
- [ ] T012 [US1] Write serialization emit-proof test `a_refusal_event_serializes_the_proposal_verbatim` in `crates/cloudkitty-core/src/events.rs` tests: record REAL events from a driven world (no hand-written fixture — rule 5's third lie) covering BOTH `absorbed` values at the ring layer (FR-008 as amended — one taxed, one absorbed), pin the taxed event's JSON including the always-present `absorbed` key, round-trip it (pins the contract's wire shape)
