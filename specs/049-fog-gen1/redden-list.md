...
| # | Finding | Disposition |
|---|---|---|
| 1 | MEDIUM — four `evals/v2` exams carry `kitty_slots` 7/5, so `observation_len` is 590/466 and `policy.rs` refuses a 404-wide artifact at load; `kitty-eval --suite evals/v2 --artifact` dies before a tick on 4 of 6 exams | OWNER FLAG 5 reaffirmed with the concrete failure mode. Not 049's to fix (the exams' rosters are the exam designs); options for the owner post-049: re-cut `scale`/`mixed-roster-*` at roster 5 so a served-width mind sits them, or accept exam-specific minds. Recorded in the PR body. **RULED 2026-09-04 (owner: "option 1 is fine")**: `evals/v3`, the four wide exams re-cut at roster 5, own spec after the PR and before the step-7 smoke; BACKLOG P1 entry. |
| 2 | MEDIUM — the one enforced welfare gate pins r = 64; the r = 5 readings are `#[ignore]`d, so a blind-cat-only regression lands green | OWNER CALL. SC-005 rules the r = 5 welfare a reading, not a gate (the Gen 1 bar is step 5's). Since T092 the SERVED world holds the 2.x bounds at r = 5 AND r = 64 (0 / 0, re-read this sitting, 6.7 s for both runs), so promoting `served_world_fog_r5_welfare_reading_with_global_vision_control` to enforced is cheap and would be a kept-behaviour guard, not a Gen 1 bar. The gate's stale comment (13 violations, the heading rule) rewritten to the T088/T092 numbers @ c51189c. **RULED 2026-09-04: NOT a gate — recommendation withdrawn.** The owner's point: the 2.x bounds were calibrated under global vision and 3.x is expected to carry a welfare hit; enforcing them post-wall builds a gate that a ruled smaller radius or a reward retune would legitimately redden and then have to weaken. The r = 5 regression tripwire is SC-011's byte-pinned 20k streams on the served roster (enforced, `reply_floor_unset_is_byte_identical`); the served welfare reading is re-run at every re-pin and its numbers recorded beside the pin (comment in `welfare_longrun.rs`). SC-005 stands as written. |
...
## Review 3 — the final scoped review (run from the Client thread at the owner's request, 2026-09-04; brief: the observation encoder vs `contracts/observation-v5.md` cell by cell, `world.rs` message enforcement + `emit_message`; write-up at the Client scratchpad `review-049-round2-2026-09-04.md`)

The offset table verified correct cell by cell by the reviewer (every
`offsets::` constant, the push order of every row, the Heard mask on
`ROW_MSG_BLOCK`, `memory_index` vs `refresh_memories`, `WANT_KINDS` /
`HERE_KINDS` order, `refusal_reason` vs `validate`). Five findings; two
engine defects fixed red-first (findings 1 and 2), one guard gap closed
red-first (finding 3), one debug assertion added red-first (finding 4),
one false comment corrected (finding 5). Every fog-era absolute pin that
the finding-1 fix moved was re-pinned from one run, with the served and
compiled welfare readings re-taken by the standing doctrine.

| # | Finding | Disposition |
|---|---|---|
| 1 | MEDIUM — `selection.rs` `priced_nearest_element`: T092's `occupied` filter dropped the visible held beam, and the remembered slot (FR-007 stores the nearest VISIBLE beam, no occupancy check, so after one environment phase it names that very tile) re-added it as the `u32::MAX` phantom; `pursue` walked the cat back into the standoff (`step_toward` refuses the last step) | REAL, and worse than stated: in real runs the filter was nearly dead (the nearest visible beam is always the remembered one). FIXED red-first (cycle r3a): the phantom is filtered by the same closure; a remembered tile outside the disc has no visible occupant and passes. Real-run effect: every sleeper whose nearest beam is held naps or cosleeps on the spot instead of walking — the ruling as written ("the nap on the spot beats a wait"). Moved: the two evolution goldens, the SC-011 preladder r = 5 streams (kitty 1 at tick 95 cosleeps with 2 where it idled in the standoff), SC-004b's first divergence (tick 15: kitty 3 naps where the pre-fog cat ATE — the score side of the same helper, sleep at cost 0 outranking eat — a held beam inside its reach; was tick 119, kitty 2 beside a held beam). The guard's T092 class widened from "a friend on a beam beside it, pre-fog waited or stepped" to "another cat on a beam inside `sunbeam_reach`, pre-fog did anything" — the beam-blocked predicate carries the rigour; FR-024(b) / SC-004b re-pointed. Welfare re-read: served 0 / 0 at r = 5 and r = 64 (unchanged); compiled 32×32 r = 5 **4 → 2** violations. **OWNER NOTE (reversible, flag 13)**: the accidental phantom walk had one good side effect — a cat walked toward a SETTLED friend's beam in reach and cosleeped on arrival; the ruling as coded now naps on the spot unless the friend is beside. If the walk-to-cosleep is wanted, it is a one-line exemption for settled occupants in `occupied`; not taken (the numbers moved the right way, and the ruling's text does not ask for it). |
| 2 | MEDIUM — `known_relief(WantEat)` read bowl presence: review 2's finding 4 mirrored on the want side; mid-tick an emptied bowl in view silenced a lawful `want_eat` from a cat with no relief | REAL; FIXED red-first (cycle r3a): one `stocked_chow_in_view` predicate now serves the here law's referent AND the want law's relief, so the two cannot drift; contract table updated. Start-of-tick verdicts unchanged (no snapshot holds an empty bowl), so the mask oracle and every start-of-tick pin stayed green as predicted. |
| 3 | LOW — `schema_five_pins.rs` asserted lengths only; every `observe.rs` row test reads through the same `offsets::` constants, so a reordered block tail keeping 404 leaves the suite green and silently reinterprets every artifact's input | REAL, and the premise CONFIRMED by cycle r3b (below): under an induced self-tail swap (memory before the own message block, constants and push order both) the other 881 tests stayed green. `the_offset_table_matches_the_contract` pins every offset and block width as literals; the contract's derived-numbers table gained the three rows. |
| 4 | LOW — `message_legal(kitty, …, view)` couples `kitty.memory` with `view.observer`; nothing enforced `kitty.id == view.observer` | REAL, latent (every caller satisfies it); `debug_assert_eq!` at the head, guarded by a `#[should_panic]` test seen failing to panic first (cycle r3a). |
| 5 | LOW — `attn.rs` pad-rule comment: "every recency there is > 0" is false; `heard_unseen` admits `wait_for_me`, outside `HEAD_KINDS`, so a friend heard only through the yield word from the observer's own tile is an all-zero live Heard row masked as padding | REAL, narrow, ACCEPTED: the comment now states the true invariant and names the one exception (rare; the MLP head reads the same zeros; the numpy oracle's rule is the same "all zero"). Excluding `wait_for_me` from `heard_unseen` would move the heard rows, scripted targeting and the here law together — not a LOW's to do without a ruling. |
| — | Perf items re-raised and withheld (already BACKLOG P2) | The reviewer's extra fix shape (ONE view per apply slot threaded into `apply_message`) added to the P2 entry. |

### Cycles r3a–r3c

| cycle | guard | prediction | observed | restore | count |
|---|---|---|---|---|---|
| r3a | three new guards on the unchanged engine: `needs_driven::tests::a_remembered_beam_under_an_awake_cat_is_not_worth_walking_to_either` (beam two tiles east under an IDLE friend, remembered), `meow::tests::want_eat_is_not_silenced_by_an_emptied_bowl_in_view` (eat armed and top, no memory, an empty bowl in view), `meow::tests::message_legal_refuses_a_kitty_that_is_not_the_views_observer` (`#[should_panic]`, kitty 2 with kitty 1's view) | RED ×3: the first at `sunbeam_worth_walking` = `Some((12,10), 2.0)`; the second at "an emptied bowl in view is not relief"; the third by NOT panicking | RED exactly there, all three (0 passed / 3 failed) | fixes landed @ 151148f; targeted GREEN; full suite 877 / **4** / 6 — the four reds all fog-era absolute pins as predicted (`golden_evolution_flag_absent_10k_ticks`, `golden_strip_witness_refusal_ring_is_the_only_delta`, `reply_floor_unset_is_byte_identical` at tick 95, `world_covering_radius_diverges_only_by_the_named_causes` at tick 15), no PreFog-era or start-of-tick pin among them | 877 / 4 / 6 |
| re-pin | SC-004b class widened (beam-blocked = another cat on a beam inside `sunbeam_reach`); goldens `a0642777…` / strip `ab932287…` from one run; `record_preladder_r5_streams` re-recorded; welfare readings re-taken | the widened class names tick 15 (kitty 3, beam-blocked) and nothing before it | "actions identical for 15 ticks; first divergence at tick 15 named (beam-blocked [2, 3]): [(2, "S-", "E")]"; served 0 / 0, compiled 2 | @ 3d331a4 | — |
| r3b | `schema_five_pins::the_offset_table_matches_the_contract` under the induced self-tail swap (`SELF_MEMORY` = 35, `SELF_MSG_BLOCK` = 55, push order swapped) | exactly ONE red, this pin at its second assertion (message block 55 ≠ 35); every other test green | RED there ("own message block 35-64", left 55); **881 / 1 / 6** — the reviewer's premise confirmed | `git checkout observe.rs`; tree clean | 881 / 1 / 6 |
| r3c | full suite after restore | 882 / 0 / 6 | **882 / 0 / 6** (wall 146 s); `cargo fmt --check` clean; clippy `-D warnings` clean; `maturin develop --release` + pytest **18 passed / 1 skipped**; binding continuity **CONTINUOUS** (`cf0cfede…` reproduced — first-legal-pair seats, untouched by the sleep arm; the mask reads the start-of-tick view, where no bowl is empty) | — | 882 / 0 / 6 |

Checked-and-cleared list from the reviewer (`blind_price`'s four-corner
test, `Lattice::axis` and `cycle_len` never zero, `prune_transient` ⊇
audibility, `reply_candidate`'s tie-break, the mask stricter than the
