...
### Session 2026-09-03 — owner rulings (post-plan clarify; timeline @ 138a289, verified)

Four items the post-plan ambiguity scan raised, hashed out with Experiments at the owner's request and ruled in the timeline doc. The plan artifacts predate this pass and are re-run after it; nothing below leans on them.

- Q: When a friend is heard but not seen, how do the want law and scripted partner targeting treat "available", given its activity state is masked? → A: Two parts. **Targeting**: a heard friend is a candidate unconditionally; idleness (not asleep, not mid-scene) is checked only on sight — walk to the stamped position, drop the target on arrival if the friend is asleep or busy; no true-state read through the fog (that would put masked state into the mask). **Want gate**: `want_cuddle` / `want_bath` / `want_play` require "no idle friend **in view**"; friends the cat can only hear never suppress the word (with a 30-tick window on five cats "none heard" would make the three words near-dead, and under fog the caller's own want is the only way its need reaches a cat that cannot see its row). The critter clause of `want_play` (ruled earlier the same day) stands. Encoded in FR-022, FR-036, US7 scenarios 8–9, SC-010.
- Q: Who writes the exploration heading, given behaviours are read-only advisors? → A: the engine, as the direction of the cat's last applied move, whatever caused it (navigation, sidestep, policy move); FR-023 continues it and the wall-within-radius rule redraws. Bias judged mild and mostly helpful (after a meal or a refuted memory the cat keeps going the way it came, the least recently swept direction; a sidestep rotates the sweep 90°). The step-5 blind-hungry span is the check; an advisor-owned heading is the upgrade if spans read long, with its own line. Encoded in FR-023 and the "Exploration step blocked" edge case.
- Q: Does the distress-gated intervention (BACKLOG P2, "definitely before fog lands") ride this wall? → A: No — its own spec on the 3.0 line, landing before the step-7 `--fresh` cutover, not inside 049 (its override state is a snapshot field, hence before the cutover). Design constraint carried to that spec's kickoff: a per-seat fallback chain (Assumptions). Cap and corpus runs keep it off. Encoded in Assumptions and the "Safeguard under fog" edge case.
- Q: Does the plugin wire version move at the wall? → A: Yes — `PROPOSAL_WIRE_VERSION` 2 → 3 in the 049 PR with a `docs/plugins.md` note: fog changes what the same shape means, and the wire grows fields. Encoded in FR-048 and SC-013.
- Q (analyze pass): which two `snapshot_resume` tests does "both tests already marked for the wall" name, given only one carries the mark in code? → A (timeline @ cefe5ba, item 5): `a_pre_041_bound_rest_duet_resumes_as_synchronized_resters` and `a_pre_028_world_resumes_and_runs`, with `pre-041-bound-duet.json` and `pre-028-world.json`; the step-3 wording was loose — the pre-028 test cannot deserialise once the kitty shims are gone. Encoded in FR-032.
- Q (analyze pass): `Meow.intensity`'s serde default is a restore shim of the same class as the seven but not in the ruled list — delete or keep? → A (item 6): DELETE with the wall; a missing field must fail at load, not read 0.0 into the digest. The pre-028 meow-entry test goes with it; its successor is the inverse guard, one literal per required field (`intensity`, `pos`, `reply`). Encoded in FR-032.

### Session 2026-09-03 — post-review rulings (T087; ruled in the Experiments session, confirmed by the owner in the Product session)

- Q: Does the idle-friend-in-view clause fit `want_bath`? → A: No — the ruling assumed bath is self-administered without the social implication. `want_bath` is an ASK: armed-only, no top-need clause, no friend gate (FR-036 amended). Cuddle and play keep the law as ruled.
- Q: Is SC-004's byte-identity through the law the right bar? → A: No — split: SC-004a the plumbing proof under a test-side pre-fog law switch; SC-004b the named-cause law (FR-024 amended).
- Q: What does the scripted groom response do with a stale ask? → A: Act only on a `want_bath` no older than the cooldown, inclusive (2.x-matching); audibility stays the one digest rule (FR-022 amended).
- Q: And with a caller it can see is already clean? → A: Decline when the caller's bath need is below the announce threshold — read off the seen row, imitable (FR-022 amended). Both rules measured: grooms of a clean caller and simultaneous groomers ~0 at every radius.
- Q (T088): The heading rule's sweep left pockets (40 corner tiles at 20×20, the whole 32×32 interior) and its coverage depended on r versus world size — how should a blind cat search? → A: The lattice serpentine tour (FR-023 re-ruled): waypoints inset floor(r/√2), spacing ≤ floor(r√2), boustrophedon and back, coverage-complete by construction; one engine-owned index per cat, no RNG draw; `explore_heading` deleted. SC-012 widens to every tile.
...
With a roster of five, each cat's view carries four kitty rows, one per friend, ordered by kitty id and never re-sorted. A friend inside the disc fills its row completely. A friend outside the disc who has spoken within the digest window keeps its message block live and its direction to the position of its last call (Q1) while its knowledge fields read zero. A friend outside the disc who has been silent is an all-zero row. `PlayKitty(slot)` therefore names the same cat every tick.
...
2b. **Given** a `needs_driven` cat with real cuddle need and a friend outside its disc who meowed inside the window from tile U, **When** it decides, **Then** the friend at U is a legal target whatever U's state (masked) and the cat walks toward U; on arrival with the friend not visible, or visible but asleep or mid-scene, the target is dropped that tick (idleness is checked only on sight — owner ruled 2026-09-03).
...
4. **Given** a world-covering radius, **When** the served roster runs all-scripted for 20,000 ticks under the pre-fog law switch, **Then** every action and every message matches the pre-fog engine's at the same seed and config (fog at infinite radius is the pre-fog world); under the ruled law the first action divergence is a named cause (the groom response declining an already-clean caller) and the message stream before it differs only where the want law (FR-036) silences a want (amended 2026-09-03, T087).
...
6. **Given** a blind cat at (10, 10) on a 20×20 world at r = 5 whose tour index names waypoint (16, 10), **When** it decides, **Then** it steps east toward it, drawing nothing from its RNG; when it stands on (16, 10) the engine advances the index at the tick's end, and its next exploring step heads for (10, 10), the next waypoint of the snake (re-ruled 2026-09-03, T088).
...
A cat may speak a want-word only when the need is armed, it is the cat's top need, and the cat has no visible or remembered relief for it (for the social words: no idle friend in view; a friend it can only hear never silences it) — so under fog a `want_eat` says "I am hungry and I do not know where food is", which no row carries. Every here-word the engine records carries a reply flag: set when a matching want from another cat was audible and the speaker could see the referent, clear for today's adjacency-law heres. A listener reads, per friend and per here-kind, whether that friend's latest here answered *its* want.
...
7. **Given** a cat whose play need is top and armed with a critter visible (or remembered) but no idle friend in view, **When** legality is evaluated, **Then** `want_play` is illegal — a known critter is known play relief (owner ruled 2026-09-03).
8. **Given** the same cat with no critter known and no idle friend in view, but a friend outside its disc who meowed inside the window, **When** legality is evaluated, **Then** `want_play` is legal — heard friends drive targeting, never the gate (owner ruled 2026-09-03).
9. **Given** a cat whose cuddle need is top and armed, with one visible friend mid-scene (or asleep) and none idle, **When** legality is evaluated, **Then** `want_cuddle` is legal; on the first tick an idle friend is in view it is illegal, adjacent or not.
...
- **Safeguard under fog**: stays existence-based (Article I: a resource must exist and be reachable; not "be visible"). No fog-aware rescue — finding is the cat's job, and a rescue would teach policies that starving makes food appear. The distress-gated intervention (BACKLOG P2) is its own spec on the 3.0 line, landing before the step-7 cutover and not in 049 (owner ruled 2026-09-03; see Assumptions). Anchor distress at the pinned radius is a radius finding.
...
- **Heard-friend arrival** (FR-022's arrival rule, restated): a scripted cat that reaches a heard-unseen friend's last-meow position and does not see the friend, or sees it asleep or mid-scene, drops the target that tick (a fresh decision follows). A heard friend is a candidate unconditionally; idleness is checked only on sight, never read through the fog. A heard position is never walked to twice without a new meow — the position is the meow's, not the cat's.
- **Want law at a world-covering radius**: relief is always visible when it exists, so the element want-words go silent in a world that always has chow and water — the ruled consequence, not a defect (the word means "I cannot see it"). `want_bath` is an ask and stays speakable whenever armed (T087).
...
- **Exploration RNG**: none — the tour index is set at generation from the kitty id and advanced on reach; the exploring step draws nothing (T088). At a world-covering radius with the served element minimums, exploration never triggers, so the action stream stays identical to the pre-fog engine (FR-024).
- **Exploration on a narrow world**: an axis too short for two inset waypoints collapses to its middle tile, so the lattice degenerates to a line or a single waypoint; the cycle still covers the world (T088).
- **Exploration step blocked**: the step toward the waypoint uses the existing step rule (occupied-tile and water-avoiding sidestep). A waypoint another cat stands on cannot be landed on, so the engine advances the index when the cat stands beside a held waypoint (T088); the index is engine-owned, never a behaviour's to write (owner ruled 2026-09-03, clarify item 2).
...
- **FR-022**: Built-in target picking runs over the FR-021 set. **Elements**: candidates are the visible elements ∪ the one remembered tile per kind; a remembered tile is walked to as if it were the element; when it comes inside the disc it is confirmed (the element is there) or refuted (memory cleared), and a refuted memory drops the cat into FR-023 exploration on the same ladder that tick. **Friends** (cuddle, play, the groom response): candidates are visible friends passing the existing availability / conscriptability test for that scene kind ∪ heard-unseen friends at their last-meow position, the latter unconditionally — their state is masked and is never read through the fog (owner ruled 2026-09-03: idleness is checked only on sight); on arrival, visible and available → proceed, not visible or asleep or mid-scene → drop the target this tick. The "take what is here" rung, sunbeam-worth-walking, and the water-avoiding step rule run over the same set. **The groom response (owner ruled 2026-09-03, T087)** acts only on a `want_bath` no older than the cooldown, inclusive (audibility stays the one 30-tick digest rule; the rung declines stale asks — a still-needy caller re-emits every cooldown, so an older ask means the need was met or the caller is mid-scene), and, on sight, declines a caller whose bath need is below the announce threshold (read off the seen row, so imitable). Measured: with both rules, grooms of an already-clean caller and simultaneous groomers fall from ~24–31 and 13–18 per 1,000 ticks to ~0 at every radius, the dirty-target rate unchanged.
- **FR-023** *(re-ruled 2026-09-03, T088)*: When a built-in cat needs an element kind that is neither visible nor remembered, it walks the **lattice serpentine tour**: waypoints on a square lattice inset floor(r/√2) from each wall with neighbouring waypoints at most floor(r·√2) apart (the two constraints for Euclidean-disc coverage of a rectangle — a corner tile within r of the corner waypoint, every interior tile within r of the nearest waypoint), visited in boustrophedon order and back, so every tile of any rectangle at any radius is inside the disc at some waypoint by construction. Each cat carries one saved state field, `explore_waypoint`, its index into the tour cycle: set by the engine at generation to `id mod cycle length` (cats spread over the tour; no RNG draw — the ruling allows at most one) and advanced by the engine in the environment phase when the cat stands on its waypoint, or beside it while another cat occupies the tile; a behaviour reads it and never writes it. On an exploring turn the cat takes one step toward its current waypoint with the existing step rule; the tour persists across errands, so a cat resumes toward the same waypoint after every one. Idling (today's no-element branch) and the memoryless `wander` are not used for this case. *History*: the 2026-09-02 ruling was a persistent heading redrawn when the wall ahead was within r; its coverage was a function of r versus world size (the 20×20 core left unseen: 100/36/4/0 tiles at r = 2/3/4/5, the corners 40 tiles at r = 5, the 32×32 interior never), which would have made the step-5 radius screen measure the sweep rule instead of vision.
- **FR-024** *(amended 2026-09-03, T087 — owner ruled)*: **(a) the plumbing.** Under the engine's test-side `WantLaw::PreFog` switch (the 2.x armed-only word law and the 2.x groom response; `#[serde(skip)]`, no config can set it) a world-covering radius with `reply_intensity_floor` unset and `announce_here` = 0 MUST reproduce the pre-fog engine's action AND message streams byte for byte at the same seed and config — the proof the visibility filter changes nothing but visibility, kept reproducible after merge. **(b) the law.** Under the ruled law the same run MAY part from the pre-fog streams only for a named cause: the message stream by wants silenced by the top-need or known-relief clauses (and calls freed by a silenced predecessor's cooldown), the action stream by the groom response's rules (FR-022) — and the proof run names the first action divergence, proves it is that, and proves nothing else moved before it. Literal byte-identity through the law is not a bar: FR-036 is a deliberate behaviour change.
...
- **FR-032**: `explore_waypoint` (FR-023; `explore_heading` until T088) and the meow record's `reply` and `pos` (FR-040) ride the step-4 snapshot bump. The snapshot restore shims are deleted: the seven in `kitty.rs`, `Pursuit.improved_at`'s default, the eighth shim `Meow.intensity`'s serde default (owner ruled 2026-09-03: under fog intensity is an observed digest feature and the ladder's tie-breaker, so a silent 0.0 on a missing field would corrupt the digest instead of failing at load), the pre-041 duet fixture and the two fixture-loading `snapshot_resume` tests (`a_pre_041_bound_rest_duet_resumes_as_synchronized_resters`, `a_pre_028_world_resumes_and_runs`) with their fixtures; the pre-028 meow-entry test is replaced by its inverse — one JSON literal per required meow field (`intensity`, `pos`, `reply`) asserting the entry fails to deserialise. A pre-3.0 save does not load.
...
- **FR-036 (want law)**: A want-kind is legal for a cat iff (a) its grounding need is armed (the existing `announce_threshold` + `announce_hysteresis` knobs; no per-kind thresholds), (b) that need is the cat's top need (`NeedKind::ALL` order breaks exact ties — the existing highest-pressure rule), and (c) the cat has no known relief for it: `want_eat` — no chow visible or remembered; `want_drink` — no water visible or remembered; `want_cuddle` — no **idle friend in view**: a visible friend whose activity is Idle (no scene running, not asleep — the activity the observer's row already carries; adjacency is not required, so this is neither the engine's "available" (adjacent) nor "conscriptable" (adjacent ∧ idle) predicate but idle-without-adjacency read off the visible row); friends the cat can only hear never suppress the word (owner ruled 2026-09-03, clarify item 1: heard friends drive targeting, never the gate); `want_play` — the same friend clause AND no critter visible or remembered (owner ruled 2026-09-03: play is always satisfiable by the solo pounce, so the word means "nothing better than solo is known", and the `want_play ↔ here_critter` pair only makes sense if a critter the caller can see silences the call; bugs and greebles are memory kinds, refuted on sight); `want_sleep` — need-only-when-top, no knowledge gate (the spec's pick between the two ruled options: sleep has no referent, and the `want_sleep → here_sunbeam` reply pair presupposes the word is speakable). Cooldown and vocabulary gates are unchanged. There is ONE predicate, used by the mask and by the built-in announce rule alike. **Amended 2026-09-03 (T087, owner ruled): two word classes.** The announcements — eat, drink, sleep, play, cuddle — follow (a), (b) and (c). `want_bath` is an ASK and is armed-only: no top-need clause, no friend gate. Its relief source is in-place self-grooming, and the partnered groom only a GROOMER can start — the groom response starts it on hearing the word — so an idle friend in view is a groomer who has to be asked, not relief the caller can execute; and no here-word answers `want_bath`, so the top-need clause (the reply ladder's honesty rule) buys nothing there. Measured on the served roster over 20,000 ticks (dirty-target partnered grooms per 1,000 ticks, with the FR-022 groom-response rules): the first-landed law 0.0 at global vision / 0.25 at r = 5; armed-only 3.1 / 4.8, at or above the pre-fog 2.0 / 3.7; `want_bath` volume 4.9 per 1,000 at global vision — one word per ~200 ticks on five cats.
...
**Plugin wire** (owner ruled 2026-09-03, clarify item 4)

- **FR-048 (wire version)**: `PROPOSAL_WIRE_VERSION` moves 2 → 3 in this change. The world a plugin receives is the fog view's snapshot — the same shape, fogged contents — and the wire grows fields (memory and `explore_waypoint` on kitties; `pos` and `reply` on meows). A v2 plugin assuming full sight cannot tell it sees a partial world, so the version says so; a plugin that refuses the version falls back per Article IV (safe; no third-party plugin is live, so the cost is the version line and a doc note). `docs/plugins.md` carries the new version and a fogged-world note. The HTTP API contract stays out of scope.
...
- **Want law**: announcements (eat, drink, sleep, play, cuddle) — armed AND top need AND no known relief (elements: visible or remembered; cuddle and play: an idle friend in view — heard friends never gate); the ask (`want_bath`) — armed-only (T087); the one predicate the mask and the built-in announce share.
...
- **Exploration tour**: the lattice serpentine tour and one saved index per cat into its cycle; the blind scripted cat's search (T088).
...
- **SC-004a**: The served roster all-scripted at a world-covering radius (reply floor unset, `announce_here` 0) under the pre-fog law switch reproduces the pre-fog engine's action and message streams byte for byte over 20,000 ticks at the same seed and config.
- **SC-004b**: The same run under the ruled law is identical up to its first action divergence; that divergence is a named cause (the groom response declining the answer to an already-clean caller — tick 550 on the served roster), and before it the message streams differ only by wants silenced by FR-036 (nothing added but cooldown-freed calls, nothing removed but want rows).
- **SC-010**: Under property test, no want-word is ever legal while its relief is visible or remembered (social kinds: while an idle friend is in view), or while it is not the top need; a heard-unseen friend never changes a want verdict; no here-word is ever legal without an adjacent referent or (audible matching want AND visible referent); every reply = 1 stamp has an audible matching want and a visible referent at emission; every heard-unseen row's position equals the friend's stamped position at its freshest audible meow; no reply is ever stamped on the tick of the want it answers.
...
- **SC-012** *(re-ruled 2026-09-03, T088)*: A blind exploring cat first sights a bowl placed on ANY tile of the served 20×20 world at r = 5 within one tour plus the approach — the tour cycle in steps (104 for the 3×3 lattice) plus width + height (144) — in every seeded trial from every tour index; the exploring step draws nothing from the RNG. Measured at landing over 399 trials: worst 108 ticks (the far corner from an unlucky index), median 28, mean 35. *History*: the heading rule's bar was one crossing (≤ 40 ticks) over the tiles its sweep could reach, which excluded 40 corner tiles.
- **SC-013**: The plugin decision request carries wire version 3 and a fogged world; the plugin end-to-end suite runs over it, and a plugin that refuses the version leaves the tick loop untouched (the existing fallback guard, re-run).
...
- **Out of scope by ruling**: codec / `ACTION_SCHEMA_VERSION` layout rules, the HTTP API contract (the plugin wire is IN — FR-048), `HEAD_KINDS` layout, the estimator/JEPA head (not wall-gated), waterline contagion (shelved, charge 0), the Here*-teacher (OUT; corpus = `announce_here = 1` seats), any client rendering of fog. Cutover housekeeping (binding-continuity re-baseline, `groom_cuddle_relief` 2.0 → 0.5 with its pin) is the wall / step-7 PRs' by ruling and is cited (SC-009), not specified.
- **Distress-gated intervention is not here** (owner ruled 2026-09-03, clarify item 3): BACKLOG P2's "definitely before fog lands" is honoured by its own spec on the 3.0 line, landing before the step-7 `--fresh` cutover (its override state is a snapshot field). Requirements carried to that kickoff: every firing stamped on the event stream with live instruments reading the stamp (refusal baseline, welfare census, collapse detector, the step-7 soak); override state in the snapshot; and the owner's design constraint — a per-seat fallback *chain*, rung = (behaviour, descend trigger, hand-back condition), snapshot holding the current rung + entry tick, the stamp carrying the rung; Gen 1 builds two rungs (masked policy → `needs_driven`), a later LLM tier is a prepended rung, not a rewrite. Cap and corpus runs keep it off. Nothing in 049 depends on it.
- **Sequencing**: this is step 4. The distress-gated intervention spec lands on the 3.0 line between this wall and the cutover. The 3.0 cutover deploy is `update.sh --fresh` at step 7 after step 5's shakeout and step 6's lock; nothing here deploys.
