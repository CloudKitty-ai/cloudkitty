...
- **Exploration step blocked**: the step along the heading uses the existing step rule (occupied-tile and water-avoiding sidestep). The heading is engine-recorded from the cat's last applied move, whatever caused it — navigation, sidestep, policy move (owner ruled 2026-09-03, clarify item 2; a read-only advisor cannot write cat state) — so a sidestep or a navigation move also sets it — after a refuted memory the cat carries on the way it was walking, and a sidestep rotates the heading; the redraw rule is unchanged.
