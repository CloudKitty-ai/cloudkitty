...
- [ ] T008 **(DEFERRED to Phase 4, lands immediately before T024 — 2026-09-03: flipping the default alone reddened 16 schema-4 derived literals across rl/server (menu 34, obs 225, 23 tokens, the 225-shaped artifact fixtures), which are exactly T025/T029/T031's must-fail set; the default, the roster check and its test move with the width so every phase lands green. The old `kitty_slots == 3` guard was observed red once already (redden cycle 1) and is re-observed when this lands.)** `crates/cloudkitty-rl/src/config.rs`: `kitty_slots` default 3 → 4 (line 84); roster ≤ `kitty_slots + 1` check in `load_configs_from_str` (line 387) naming both numbers (FR-011); the existing `assert_eq!(rl.observation.kitty_slots, 3)` at line 414 is a kept-behaviour guard that MUST go red first, then point it at 4; new test `roster_above_slots_plus_one_is_refused`; red-first `>` → `>=`
