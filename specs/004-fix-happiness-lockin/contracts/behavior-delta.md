...
|-------|-------------------------------|
| `pursuit` | Your current chase, as the engine saw it actually happen: target, the tick it started, best distance achieved. Survives one-tick detours; cleared by catching the target, chasing something else, the target dying, or abandonment. You cannot write it — only chasing does. |
...
`chase_patience_ticks` elapsed against it without improving on
`pursuit.closest`. When no viable partner is within `solo_play_reach` and
play ≥ safeguard, the built-ins propose solo play.
