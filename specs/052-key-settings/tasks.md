...
- [X] T024 Open the PR from `052-key-settings` against `main` with the house body (what / why / behavior table from contracts §4 / verification table from `redden-list.md` with predicted vs observed / not covered: the live deploy print is the box's next deploy); wait for CI treating "no checks reported" as pending; report to the owner. Merge only on the owner's word.
