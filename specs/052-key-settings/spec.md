...
- **FR-009**: If `/settings` is not served by this binary, or returns an empty or unreadable body, or the server no longer answers at all (three connect attempts), or the section cannot be written to stdout, the script MUST print a message that names which it was and MUST exit non-zero with a status distinct from the rollback branch's (2), without rolling back and without skipping the `deployed` line or the backup prune.
