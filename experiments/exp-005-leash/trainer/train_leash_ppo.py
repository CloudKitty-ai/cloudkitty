...
EXP005 = _EXPERIMENTS / "exp-005-leash"
# this dir's data.py is the CLONE shim -- it must not shadow exp-004's
# data module that ppo_env_v4 imports; drop the script dir from the path
sys.path[:] = [q for q in sys.path if not q or Path(q).resolve() != HERE]
...
sys.path.insert(1, str(_EXPERIMENTS / "attn-ppo-2026-08-13"))
...
def schedules(update, total_updates, args):
    ...
    # exp-005: anneal beta0 -> beta_inf over the first 20%, then HOLD.
    # beta_inf=0 reproduces the A1 recipe expression exactly (control).
...
def main():
    ...
    ap.add_argument("--kl-beta-final", type=float, required=True,
                    help="beta_inf: the held final KL weight (0/0.05/0.2)")
