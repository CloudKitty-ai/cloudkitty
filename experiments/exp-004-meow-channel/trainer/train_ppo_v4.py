"""exp-004 MAPPO fine-tune: the two-head generation (prereg §3/§5).
...
def main():
    ...
    ap.add_argument("--clone", type=Path,
                    default=EXP / "artifacts/clone/clone.pt")
    ...
    obs_dim, mask_dim = runner.dims
    if policy.dims[0] != obs_dim or policy.dims[-1] != mask_dim:
        sys.exit(
    ...
    (out / "run-manifest.json").write_text(json.dumps({
        ...
        "init": "clone-v4", "init_sha256": anchor_sha,
