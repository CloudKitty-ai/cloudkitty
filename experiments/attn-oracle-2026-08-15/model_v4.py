"""EntityPolicy at the spec-033 surface: 30 tokens, head 16, 50 logits.
Identical architecture to the certified v3 forward; only the msg token
count (8->15), type-embedding table (15->22 rows), and message head
(9->16) widen.
"""
import torch
from torch import nn

from obs_tokens_v4 import (CHOW_F, CLOCK_F, CRIT_F, KITTY_F, MSG_F, N_CHOW,
                           N_CRIT, N_KITTY, N_MSG, N_SUN, N_WATER, SELF_F,
                           SUN_F, WATER_F, tokenize_obs)

N_ACT, N_MSGHEAD = 34, 16
DENSE_ACT = [0, 1, 2, 3, 4, 8, 12, 16, 17, 25, 33]
KITTY_MENU = torch.tensor([[5 + k, 9 + k, 13 + k, 22 + k, 30 + k]
                           for k in range(N_KITTY)])
CRIT_MENU = torch.tensor([[18 + j, 26 + j] for j in range(N_CRIT)])


class EntityPolicyV4(nn.Module):
    ...
    def forward(self, obs):
        ...
        xs, ms = [], []
        for name in order:
            xs.append(self.embed[name](toks[name]) + self.type_emb[trows[name]])
            ms.append(pads[name])
