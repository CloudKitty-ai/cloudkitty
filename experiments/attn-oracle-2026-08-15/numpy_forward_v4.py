"""Independent numpy reference forward for the spec-033 surface —
adapted from the certified numpy_forward_v3 with only the widths moved
(15 msg kinds, 22 type rows, head 16). Reads the artifact's own bytes.
"""
import json
import struct
...
def load_artifact(buf):
    ...
    assert header["observation_schema"] == 4
...
def numpy_forward(p, obs):
    ...
    act[:, np.array(KITTY_MENU).flatten()] = (
        x[:, 1:4] @ p["kptr.w"].T + p["kptr.b"]).reshape(n, -1)
    act[:, np.array(CRIT_MENU).flatten()] = (
        x[:, 10:14] @ p["cptr.w"].T + p["cptr.b"]).reshape(n, -1)
